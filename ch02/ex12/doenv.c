...
void replace_environ(int, char**);
void append_environ(int, char**);
...
int main(int argc, char *argv[]) {
	...
	if (argc == 1) {
		...
		replace_environ(argc, argv);
	}
	else {
		append_environ(argc, argv);
	}
...
void replace_environ(int argc, char** argv) {

	// Offset pointers by "command [-i]"
	char **tailp = argv + 2; 
	char **headp = argv + 2;

	char var_s[250], value_s[250], empty_s[250];
	...
	char **environ_new = malloc( sizeof(utilp - headp) );
	int i;
	for (i=0; i< utilp - headp; i++) {
		environ_new[i] = malloc( sizeof(argv[i+2]) );
		environ_new[i] = argv[i+2];
	}
	environ = environ_new;
	...
	// Otherwise, print env
	if (utilp != NULL) {
		char *util = malloc( sizeof(tailp - utilp) + sizeof(char) * (tailp-utilp) );
		while ( *utilp != NULL ) {
			strcat(util, *utilp);
			strcat(util, " ");
			utilp++;
...
void append_environ(int argc, char** argv) {
	int offset = 1;

	// Offset pointers by "command"
	char **tailp = argv + offset; 
	char **headp = argv + offset;
